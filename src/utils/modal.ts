...
export const createModal = (task: Task) => {
  ...
  modal.innerHTML = `
        ...
        <form id="task-form">
            ...
            <input type="date" id="taskDueDate" name="due-date" required value="${
              (task.getDueDate() as Date).toISOString().split("T")[0]
            }">
            <label for="taskPriority">Priority:</label>
            <select id="taskPriority" name="priority">
              <option value="high" ${
                task.getPriority() === 0 ? "selected" : ""
              }>Haute</option>
              <option value="medium" ${
                task.getPriority() === 1 ? "selected" : ""
              }>Moyenne</option>
              <option value="low" ${
                task.getPriority() === 2 ? "selected" : ""
              }>Basse</option>
            </select>
    ...
    `;
  modal.querySelector(".close")?.addEventListener("click", (): void => {
    modal.remove();
  });
  modal.addEventListener("click", ({ target }: Event): void => {
    if (target === modal) modal.remove();
  });
  alterFormSubmit(
    modal.querySelector<HTMLFormElement>("#task-form") as HTMLFormElement,
    (form) => {
      onFormSubmit(form);
      task.suicide();
      modal.remove();
